      ...
      ShellRoute(
        ...
        routes: [
          ...
          GoRoute(
            ...
            builder: (_, state) {
              final extra = state.extra;
              // New format: {'path': '...', 'mode': 'coin'|'character'}
              // or batch:   {'paths': [...], 'mode': '...'}
              if (extra is Map<String, dynamic>) {
                final mode = extra['mode'] as String? ?? 'coin';
                final paths = extra['paths'] as List<String>?;
                final singlePath = extra['path'] as String?;
                if (paths != null) {
                  return DetectionScreen(imagePaths: paths, scanMode: mode);
                }
                return DetectionScreen(imagePath: singlePath ?? '', scanMode: mode);
              }
              // Legacy formats
