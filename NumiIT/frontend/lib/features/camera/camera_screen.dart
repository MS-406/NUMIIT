...
import 'widgets/scan_mode_sheet.dart';
...
class _CameraScreenState extends ConsumerState<CameraScreen>
  ...
  Future<void> _capture() async {
    ...
    try {
      ...
      if (!mounted) return;
      await _navigateWithModeSelection(saved);
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _gallery() async {
    ...
    if (paths.length == 1) {
      await _navigateWithModeSelection(paths.first);
    } else {
      // For batch: show mode sheet once, apply to all
      final mode = await ScanModeSheet.show(context);
      if (mode == null || !mounted) return;
      context.push(
        '/detection',
        extra: {'paths': paths, 'mode': mode.name},
      );
    }
  }

  /// Shows scan mode selection sheet, then navigates to detection.
  Future<void> _navigateWithModeSelection(String imagePath) async {
    final mode = await ScanModeSheet.show(context);
    if (mode == null || !mounted) return;
    context.push(
      '/detection',
      extra: {'path': imagePath, 'mode': mode.name},
    );
  }
