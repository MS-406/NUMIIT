...
import 'widgets/era_confidence_panel.dart';
...
class _DetectionScreenState extends ConsumerState<DetectionScreen> {
  ...
  Future<void> _runAnalysis() async {
    ...
    final modeEnum = widget.scanMode == 'character' ? ScanMode.character : ScanMode.coin;
    final scan = await ref
        .read(scanProcessingProvider.notifier)
        .processImage(_activePath!, mode: modeEnum);
          ...
          : SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (processing.eraScores.isNotEmpty) ...[
                    const SizedBox(height: AppSpacing.md),
                    EraConfidencePanel(eraScores: processing.eraScores),
                  ],
                  ...
                  const SizedBox(height: AppSpacing.lg),
                  GoldButton(
                    label: (processing.isComplete && (processing.regions.isEmpty || (processing.eraScores.isNotEmpty && processing.eraScores.first.confidence == 0.0)))
                        ? 'Retake'
                        : 'Translate All Regions →',
                    onTap: processing.isComplete 
                        ? () {
                            if (processing.regions.isEmpty || (processing.eraScores.isNotEmpty && processing.eraScores.first.confidence == 0.0)) {
                              context.pop();
                            } else {
                              _translate();
                            }
